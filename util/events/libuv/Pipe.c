...
static uint8_t sendMessage2(struct Pipe_WriteRequest_pvt* req)
{
    ...
    int ret = uv_write(&req->uvReq, (uv_stream_t*) &pipe->peer, buffers, 1, sendMessageCallback);
    if (ret) {
                        ...
                        struct Allocator* userAlloc))
{
    ...
    struct Jmp jmp;
    Jmp_try(jmp) {
        out = newPipeAny(eb, buff, &jmp.handler, userAlloc);
    } Jmp_catch {
        Er_raise(out->alloc, "Error in newPipeAny: [%s]", jmp.message);
    }
                               ...
                               struct Allocator* userAlloc))
{
    ...
    struct Jmp jmp;
    Jmp_try(jmp) {
        out = newPipeAny(eb, pipeName, &jmp.handler, userAlloc);
    } Jmp_catch {
        Er_raise(out->alloc, "newPipeAny failed: [%s]", jmp.message);
    }
