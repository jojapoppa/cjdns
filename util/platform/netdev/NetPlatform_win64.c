...
NET_LUID NetPlatform_win64_getLuid(const char* name, struct Allocator* alloc);
NET_LUID NetPlatform_win64_getLuid(const char* name, struct Allocator* alloc)
{
    uint16_t ifName[IF_MAX_STRING_SIZE + 1] = {0};
    MultiByteToWideChar(CP_ACP, 0, name, strlen(name), ifName, IF_MAX_STRING_SIZE + 1);
    NET_LUID out;
    ConvertInterfaceAliasToLuid(ifName, &out);
    return out;
}
...
void NetPlatform_flushAddresses(const char* deviceName, struct Allocator* alloc, struct Except* eh)
{
    NET_LUID luid = NetPlatform_win64_getLuid(deviceName, alloc);
    ...
    FreeMibTable(table);
    if (ret) {
        printf("error DeleteUnicastIpAddressEntry %ld\n", ret);
        return;
    }
    ...
    FreeMibTable(table);
    if (ret) {
        printf("DeleteUnicastIpAddressEntry %ld\n", ret);
    }
}
#include "util/Hex.h"
#include <stdio.h>
                            ...
                            struct Allocator* tempAlloc))
{
    ...
    ipRow.InterfaceLuid = NetPlatform_win64_getLuid(interfaceName, tempAlloc);
